        ...
        for _ in range(10):
            ...
            if aggregation=="count":
                ...
                for k1 in range(queries[i - 1][0][0], queries[i - 1][0][1] + 1):
                    for k2 in range(queries[i - 1][1][0], queries[i - 1][1][1] + 1):
                        for j in oracle.keys():
                            count_value+=oracle[j].count([k1,k2])
                            true_count_value += trueOracle[j][(k1,k2)]
                count_value=(count_value/12005)*n
                ...
                for k1 in range(queries[i - 1][0][0], queries[i - 1][0][1] + 1):
                    for k2 in range(queries[i - 1][1][0], queries[i - 1][1][1] + 1):
                        for j in oracle.keys():
                            sum_value += j*oracle[j].count([k1,k2])
                            true_sum_value += j*trueOracle[j][(k1,k2)]
                sum_value=(sum_value/12005)*n
...
if __name__ == '__main__':
    oraclePath = "experiments//energy_2_gr_PGRR_results.txt"
    oracleInterval = 129
    ...
    ans,trueans, relativeError, averageError = query_on_adult_dim2(oraclePath, oracleInterval,
                                                           ...
                                                           trueOraclePath,
                                                       aggregation="count")
    # file_1 = open("experiments//energy_2_gr_PGRR_results.txt", "r")
    # a = file_1.readline()
    # print(a)

    print(relativeError)
    with open("experiments//final_energy_2_count_range_gr_PGRR.txt", "w+") as f:
        f.write(str(ans) + "\n")
    ...
    ans,trueans, relativeError, averageError = query_on_adult_dim2(oraclePath, oracleInterval,
                                                           ...
                                                           aggregation="sum")
    print(relativeError)
    with open("experiments//final_energy_2_sum_range_gr_PGRR.txt", "w+") as f:
        f.write(str(ans) + "\n")
        f.write("true ans" + str(trueans) + "\n")
        f.write("relativeError:" + str(relativeError) + "\n")
        f.write("averageError:" + str(averageError) + "\n")
