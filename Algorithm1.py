...
def framework(S,groupDict,dict,budget,Ycardinality,delta,filepath,dimension,auctionMethod=None,randomiser=None):
    ...
    if groupDict is None:
        ...
        #         f.write("table without scaler:" +str(ansTable)+"\n")
            ...
            if len(S) != 0:
                fvalues = OLH.queryMechanism(cur_S, delta, Ycardinality, dict, dimension)
            else:
                print("len(S)==0")
                return
            with open(filepath, 'a+') as f:
            ...
            with open(filepath, 'a+') as f:
                ...
                for i in range(len(dict)):
                    ...
                    if i != len(dict) - 1:
                        f.write(", ")
                f.write("}\n")
        elif randomiser=="OLHnew":
            if len(S) != 0:
                fvalues = OLHnew.queryMechanism(cur_S, delta, Ycardinality, dict, dimension)
            else:
                print("len(S)==0")
                return
            with open(filepath, 'a+') as f:
