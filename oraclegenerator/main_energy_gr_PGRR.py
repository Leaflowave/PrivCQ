...
def energy_main_exp1(delta,budget,repeatTimes):
    ...
    framework(S,groupDict,sequence,budget,len(sequence),delta,"experiments//energy_1_gr_PGRR_results.txt",[0],randomiser="PGRR",auctionMethod="GR")
    end = time.time()
    print(end-start)
    return
...
def energy_main_exp2(delta,budget,repeatTimes):
    ...
    framework(S, groupDict, sequence, budget, len(sequence), delta, "experiments//energy_2_gr_PGRR_results.txt", [0,1],randomiser="PGRR",auctionMethod="GR")
    end = time.time()
    print(end - start)
    return
...
def energy_main_exp3(delta,budget,repeatTimes):
    ...
    framework(S, None, sequence, budget, len(sequence), delta, "experiments//energy_3_gr_PGRR_results.txt", [0, 1,2],randomiser="PGRR",auctionMethod="GR")
    end = time.time()
    print(end - start)
    return
...
def energy_main_exp4(delta,budget,repeatTimes):
    ...
    framework(S, None, sequence, budget, len(sequence), delta, "experiments//energy_4_gr_PGRR_results.txt", [0, 2],randomiser="PGRR",auctionMethod="GR")
    end = time.time()
    print(end - start)
    return
